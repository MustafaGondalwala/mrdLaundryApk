  ...
  register = async () => {
    ...
    if(this.state.validation){
        ...
        await axios({
          ...
          data:{ customer_name: this.state.customer_name, phone_number: this.state.phone_number, email: this.state.email, fcm_token:this.state.fcm_token }
        })
        .then(async response => {
      ...
      <ScrollView keyboardShouldPersistTaps='always'>
        <View style={styles.container}>
          ...
          <View style={styles.login_container} >
            <Text style={{ color:colors.theme_fg, fontSize:16 }} onPress={this.handleBackButtonClick} >{strings.already_i_have_an_account}</Text>
          </View>
        </View>
      </ScrollView>
    )
